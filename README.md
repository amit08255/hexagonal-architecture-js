...
* **Ports and data flow:** Every app component or sub-app communicate with use cases through input ports. App components receive data from input port which is then provided to specific use case. Data received from use case input port, is processed and if required is transferred to output port. Output port uses output adapter to receive data from outside sources such as - REST APIs, GraphQL, LocalStorage etc. Output adapter uses repositories which acts as ways to receive data from outside sources. Output port should return promise when used so that it is consistent in different ways of data fetching. When promise returned by output port is resolved, the data retrieved is sent to output port. Use cases communicates with outside world through output ports.

* **Ports and events:** Communication to and from use cases are done using port events. To communicate with use cases, app components emits an event which is captured by event handler inside input port. Once processing and retrieving data, use cases communicate through output port which emits event that can be captured by even handlers in our app components.

* **Output adapter and repository:** When data is required from external sources, use cases need output port. Output port uses output adapters which are used to retrieve data. Output adapter uses repository to fetch data, when data is provided from repository, it then should be processed by output adapter to convert into format expected by use case.
